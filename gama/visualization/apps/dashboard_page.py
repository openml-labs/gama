...
from gama.visualization.apps.plotting import (
    individual_plot,
    aggregate_plot,
    plot_preset_graph,
)
...
plot_control_container = html.Div(
    ...
    children=[
        html.Div(
            [html.Label("x-axis"), dcc.Dropdown(id="x-axis-metric")], style=third_width
        ),
        html.Div(
            [html.Label("y-axis"), dcc.Dropdown(id="y-axis-metric")], style=third_width
        ),
        html.Div(
            [
...
dashboard_page = html.Div(
    id="dashboard-main", children=[visualization_container, report_select_container]
)
...
@dash_app.callback(
    Output("update-trigger", "data"), [Input("update-timer", "n_intervals")]
)
...
@dash_app.callback(
    [Output("x-axis-metric", "options"), Output("y-axis-metric", "options")],
    ...
    if preset_value == "custom":
        if logs is None or logs == [] or xaxis is None or yaxis is None:
            ...
            if aggregate == "separate-line":
                plots = [
                    individual_plot(reports[log], xaxis, yaxis, mode) for log in logs
                ]
        ...
        return plot_preset_graph(
            [reports[log] for log in logs], filtered_aggregate, preset_value, aggregate
        )
    else:
        return {}
