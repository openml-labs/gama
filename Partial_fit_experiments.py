...
for i in range(initial_batch + 1, len(B)):
    ...
    print(
        f"New data point predicted is {i} with predicted label {y_pred} and real label {y[i]}"
    )
    ...
    Online_model = Auto_pipeline.model
    print(
        f"Online model is {Auto_pipeline.model} and hyperparameters are: {Auto_pipeline.model._get_params()}"
    )
