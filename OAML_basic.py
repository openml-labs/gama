...
for i in range(initial_batch+1,len(B)):
    ...
    if (drift_detector.detected_change()) or ((i - last_training_point) > 50000):
        ...
        Perf_current = evaluate.progressive_val_score(dataset, Online_model, metrics.Accuracy())
        print(Perf_current)
        Perf_new = evaluate.progressive_val_score(dataset, Auto_pipeline.model, metrics.Accuracy())
        print(Perf_new)
